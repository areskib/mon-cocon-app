          ...
          <tr>
            <td style="padding:36px 32px 8px;">
              ...
              <p style="margin:0 0 16px; font-family: Georgia, serif; font-size:15.5px; line-height:1.7; color:#4C342E;">
                Bravo, et merci pour ta confiance ! Ta cure vient d'être validée personnellement par mes soins — ton parcours démarre officiellement.
              </p>

              <p style="margin:0 0 16px; font-family: Georgia, serif; font-size:15.5px; line-height:1.7; color:#4C342E;">
                Je m'appelle <strong>Lindsay</strong>, je suis ta conseillère — c'est moi qui vais t'accompagner tout au long de ce programme. Tu as maintenant accès à ton espace personnel dans <strong>Mon Cocon</strong>, où tu retrouveras ta cure, ton alimentation adaptée à ton profil, ton programme sportif, et un suivi ultra-personnalisé au fil des semaines.
              </p>
                ...
                <tr>
                  <td style="padding:18px 20px;">
                    <p style="margin:0; font-family: Georgia, serif; font-style:italic; font-size:14.5px; line-height:1.6; color:#4C342E;">
                      La prochaine étape, c'est notre premier rendez-vous ensemble — c'est là que tout prend vraiment forme.
                    </p>
                  </td>
                </tr>
              </table>
...
exports.handler = async function (event) {
  ...
  // Mail de bienvenue via Resend, envoyé pile au moment de la validation (remplace
  // le dernier module du flow Make, qui envoyait via Gmail au moment de l'inscription —
  // ici c'est déclenché plus tard, une fois la cure réellement validée par Lindsay).
  // Non-bloquant : un souci d'envoi ne doit jamais faire échouer la validation elle-même.
  try {
    if (RESEND_API_KEY) {
      const qrRes = await fetch(
        `${SUPABASE_URL}/rest/v1/quiz_results?select=email,raw_data&user_id=eq.${clientUserId}&order=created_at.desc&limit=1`,
        { headers }
      );
      const qrRows = qrRes.ok ? await qrRes.json() : [];
      if (qrRows.length) {
        const email = qrRows[0].email;
        let prenom = "";
        try { prenom = JSON.parse(qrRows[0].raw_data).firstname || ""; } catch (e) {}
        if (email) {
